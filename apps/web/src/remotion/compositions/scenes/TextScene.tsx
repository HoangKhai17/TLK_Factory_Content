...
function AnimatedText({ element, startFrame, palette }: { element: TextElement; startFrame: number; palette: ColorPalette }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const layout = useLayout();
  const delay = (element.animationDelay ?? 0) * fps;
  const eff = Math.max(0, frame - startFrame - delay);
  ...
  switch (element.animation) {
    ...
    case "slideInUp":
      ...
      translateY = interpolate(eff, [0, fps * 0.5], [50, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
      break;
    case "slideInDown":
      opacity    = interpolate(eff, [0, fps * 0.4], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
      translateY = interpolate(eff, [0, fps * 0.5], [-50, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
    ...
    case "slideInLeft":
      opacity    = interpolate(eff, [0, fps * 0.4], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
      translateX = interpolate(eff, [0, fps * 0.5], [-70, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
      break;
    case "slideInRight":
      opacity    = interpolate(eff, [0, fps * 0.4], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
      translateX = interpolate(eff, [0, fps * 0.5], [70, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const content = element.animation === "typewriter"
